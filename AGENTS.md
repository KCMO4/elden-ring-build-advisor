...
| GET | `/api/builds` | Returns 30 curated community build templates for the build recommender |
...
│   └── index.ts
└── data/                 # JSONs generated by sync-data (do NOT edit manually, except builds.json)
...
├── api/
│   └── client.ts         # parseSave(), getBuilds() — typed fetch
├── hooks/
...
│   ├── buildMatcher.ts           # Build recommender: matchBuild(), rankBuilds() — weighted scoring
│   │                             # (stat similarity 0-70, level range 0-10, inventory cross-check 0-20)
...
└── components/
    ...
    ├── StatsPanel/         # 8 attributes with bars (VIG/MND/END/STR/DEX/INT/FAI/ARC) + info tooltips
    ├── DerivedStatsPanel/  # Sub-tabbed panel: Body | Attack | Defense — all stats have info tooltips
    ...
    ├── AdvisorPanel/       # Rendered in BuildPage via "Builds" tab — community build recommender
    │                       # 30 curated builds matched to player stats (weighted scoring 0-100),
    │                       # expandable cards with weapons, shields, armor, talismans, ashes, spells,
    │                       # stat comparison (yours vs ideal), inventory cross-check + Next Caps optimizer
...
N active characters → CharacterSelect → BuildPage
    ↓
BuildPage renders:
  - StatsPanel (8 attributes, softcap visual indicator, info tooltips on hover)
  - DerivedStatsPanel (sub-tabs: Body | Attack | Defense — talisman/Great Rune/Physick/buff bonuses, 2H toggle, rune calculator, info tooltips on every stat)
  - EquipmentGrid (weapons + armor + talismans, hover → ItemTooltip, spell type colors + FP badge)
  - Content tabs: Inventory | Builds | Matchmaking
    - InventoryPanel (tabs with search + type filter, hover → InventoryTooltip with armor efficiency)
    - AdvisorPanel (30 community builds ranked by stat match + inventory cross-check + next caps optimizer)
    - MatchmakingCalc (co-op & invasion level/weapon upgrade ranges)
```
